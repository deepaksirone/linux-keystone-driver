...
int keystone_get_verifiable_nonce(unsigned long data) {
	struct sbiret ret;
	struct keystone_ioctl_get_verifiable_nonce *arg = (struct keystone_ioctl_get_verifiable_nonce *) data;

	ret = sbi_sm_get_verifiable_nonce();
	arg->error = ret.error;
	arg->value = ret.value;
...
long keystone_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
  ...
  switch (cmd) {
    ...
    case KEYSTONE_IOC_GET_VERIFIABLE_NONCE:
      ret = keystone_get_verifiable_nonce((unsigned long) data);
      break;
